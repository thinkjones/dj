...
impl Permissions {
    pub fn new() -> Permissions {
        Permissions { manifest: Manifest::from_toml(include_str!("plugin.toml")).expect("permissions manifest") }
    }
}
...
impl Plugin for Permissions {
    ...
    fn doctor(&self, ctx: &PluginContext) -> Result<Health> {
        let tpl = catalog_root(ctx.cfg).join("permissions").join("template.json");
        if !tpl.exists() {
            return Ok(Health { status: HealthStatus::Missing, details: vec![format!("template not found: {}", tpl.display())] });
        }
        Ok(Health { status: HealthStatus::Ok, details: vec!["permissions template present".into()] })
    }

    fn list(&self, _ctx: &PluginContext) -> Result<Vec<String>> {
        Ok(crate::permissions::targets::all_targets().into_iter().map(|t| t.name.to_string()).collect())
    }
