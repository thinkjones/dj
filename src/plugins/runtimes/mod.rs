...
impl Runtimes {
    pub fn new() -> Runtimes {
        Runtimes { manifest: Manifest::from_toml(include_str!("plugin.toml")).expect("runtimes manifest") }
    }
...
impl Plugin for Runtimes {
    ...
    fn plan(&self, ctx: &PluginContext) -> Result<Vec<PlanStep>> {
        ...
        let mut steps: Vec<PlanStep> = runtimes.into_iter().map(|r| PlanStep {
    ...
    fn run(&self, ctx: &PluginContext) -> Result<()> {
        ...
        for r in &runtimes {
            ...
            std::process::Command::new("mise").args(["use", "-g", &spec]).status()?;
        }
        for pm in &pms {
            ...
            if !on_path {
                ...
                std::process::Command::new("sh").args(["-c", &pm.install_script]).status()?;
            }
        }
        Ok(())
    }
    ...
    fn doctor(&self, ctx: &PluginContext) -> Result<Health> {
        let (runtimes, pms) = Self::entries(ctx);
        let on_path = std::process::Command::new("which").arg("mise").output()
            .map(|o| o.status.success()).unwrap_or(false);
        if !on_path {
            return Ok(Health { status: HealthStatus::Missing, details: vec!["mise not installed".into()] });
        }
        Ok(Health { status: HealthStatus::Ok, details: vec![
            format!("{} runtimes, {} package managers in catalog", runtimes.len(), pms.len())
        ]})
    }

    fn list(&self, ctx: &PluginContext) -> Result<Vec<String>> {
        let (runtimes, pms) = Self::entries(ctx);
        let mut out: Vec<String> = runtimes.into_iter().map(|r| format!("{}@{}", r.name, r.version)).collect();
