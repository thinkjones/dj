...
impl Shell {
    pub fn new() -> Shell {
        Shell { manifest: Manifest::from_toml(include_str!("plugin.toml")).expect("shell manifest") }
    }
...
impl Plugin for Shell {
    ...
    fn plan(&self, ctx: &PluginContext) -> Result<Vec<PlanStep>> {
        Ok(Self::entries(ctx).into_iter().map(|f| PlanStep {
    ...
    fn doctor(&self, ctx: &PluginContext) -> Result<Health> {
        ...
        if !zshrc.exists() {
            return Ok(Health { status: HealthStatus::Missing, details: vec!["~/.zshrc not found".into()] });
        }
        Ok(Health { status: HealthStatus::Ok, details: vec![format!("{n} shell functions in catalog; ~/.zshrc present")] })
    }
