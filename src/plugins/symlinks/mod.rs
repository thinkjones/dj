...
impl Symlinks {
    pub fn new() -> Symlinks {
        Symlinks { manifest: Manifest::from_toml(include_str!("plugin.toml")).expect("symlinks manifest") }
    }
...
impl Plugin for Symlinks {
    ...
    fn plan(&self, ctx: &PluginContext) -> Result<Vec<PlanStep>> {
        Ok(Self::entries(ctx).into_iter().map(|s| {
            let link = catalog::expand_tilde(&s.destination).join(&s.name);
            let exists = link.exists() || link.symlink_metadata().is_ok();
            PlanStep {
                description: format!("symlink {} → {} ({})", s.name, s.target, if exists { "linked" } else { "to create" }),
                mutates: !exists,
            }
        }).collect())
    }
    ...
    fn run(&self, ctx: &PluginContext) -> Result<()> {
        ...
        for s in &entries {
            ...
            if target.exists() || target.symlink_metadata().is_ok() {
                println!("{}", format!("  linking {} → {}", link.display(), target.display()).cyan());
                std::fs::create_dir_all(&dest_dir)?;
                let _ = std::fs::remove_file(&link);
                std::os::unix::fs::symlink(&target, &link)?;
            } else {
                println!("{}", format!("  skipping {} (target {} does not exist)", s.name, target.display()).yellow());
            }
        }
        Ok(())
    }
    ...
    fn doctor(&self, ctx: &PluginContext) -> Result<Health> {
        ...
        let status = if missing > 0 { HealthStatus::Warn } else if entries.is_empty() { HealthStatus::Missing } else { HealthStatus::Ok };
        Ok(Health { status, details: vec![format!("{linked} linked, {missing} missing")] })
    }

    fn list(&self, ctx: &PluginContext) -> Result<Vec<String>> {
        Ok(Self::entries(ctx).into_iter().map(|s| format!("{} → {}", s.name, s.target)).collect())
    }
